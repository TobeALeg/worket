...
验证：`npm run build` 通过；隔离 Chromium 的合成 IPC 页面验证历史入口键盘打开、三点菜单执行操作、返回列表、实际准备/失败提示，以及 400px、360px 和长执行者名称的布局。报告与截图见 `output/playwright/panel-cleanup/`。未调用生产服务，已安装应用的进程与包哈希保持不变。
