...
当前结论：runner 已可重复执行、失败可追溯，E0 已定位至少三个明确错误族——WorkState 捕获/投影不足、Definition 替代链与范围泛化风险、冲突案例结构可靠性失败。真实案例已完成 H0 授权、H1 gold、真实 Working Context、模型 proposal 与 H3 人工风险裁定；S 轨道可发布人工确认的必需/关键召回和噪音下限，但因正向输出未全量人工复核，正式精确率仍未知，也不能宣称抽取质量或业务收益已提升。
...
本机确认页只把 5 条 Gold 覆盖和 41 条非 `USEFUL` 输出组成 46 项高风险队列，按“检查内容 → 自动判断 → 人工确认”显示；判断正确直接进入下一条，分歧时才修改分类、对应关系和原因。用户已于 2026-09-22 完成全部 46 项并以 `Dandi` 签名，最终状态为 `HUMAN_RISK_REVIEWED`；其中 2 项经过修改或补充说明。61 条 `USEFUL` 输出在最终文件中标记为 `MODEL_ONLY`，因此这条路径减少了本轮人工阅读量，但不把模型自评冒充人工证据。proposal 使用 2 次可观察 provider 调用、16,702 input tokens、51,920 output tokens；供应商仍未返回币种费用。

风险确认评分显示：S 轨道 5/5 条必需且关键 Gold 均被覆盖，必需召回与关键召回均为 100%。102 条输出中，61 条由模型判为 `USEFUL`，41 条经人工确认属于部分正确、重复或当前无关，已确认噪音下限为 40.2%。第一份案例为 9/58（15.5%）确认噪音，第二份为 32/44（72.7%），说明模型虽然覆盖了关键要求，却在长历史中保留了大量实例细节、完成动作和重复投影。由于 61 条正向输出没有逐项人工复核，正式 `useful_precision`、`noise_rate` 与 `unsupported_rate` 继续为 `null`；59.8% 只能记为风险确认后的模型估计，不能写成全量人工精确率。
