...
export function resolveReviewField(original: DefinitionContent, field: string): FieldTarget | null {
  ...
  if (section) {
    ...
    if (key && sectionItems(original, section).some(i => i.key === key)) return { section, key };
    return null;
  }
...
export function reviewFieldValue(content: DefinitionContent, original: DefinitionContent, field: string): unknown {
  const target = resolveReviewField(original, field);
