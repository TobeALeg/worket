...
type Item = DefinedItem & Partial<Pick<InputSpec, "valueType" | "required" | "choices" | "defaultValue">> & { obligation?: "REFERENCE" | "REQUIRED" };
  ...
  modal.addEventListener("click", event => {
    ...
    void (async () => {
      ...
      if (addressId) {
        ...
        if (action === "material") { const path = await window.workpet.chooseDefinitionFile(); if (closed) return; if (path) bindings[item.key] = path; }
        render(); return;
      }
