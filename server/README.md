...
为桌面 Worket 的工作沉淀与当前阶段整理提供模型调用、额度与接入凭据管理。管理页面可以在尚未配置供应商时直接启动；供应商密钥仅由后台保存。

当前阶段整理通过 `/v1/continuations` 提交，使用相同 GET/DELETE/ack 作业协议，capabilities 声明 `continuationSchemaVersions:[1]`。每次占用一次模型调用，最多 80,000 JSON 字符；输入由桌面明确授权，结果仅短期留在内存。`requests.operation` 区分整理与沉淀，旧记录默认 definition。回滚不兼容的旧服务前须备份并恢复兼容表结构；不能直接只回退代码。
